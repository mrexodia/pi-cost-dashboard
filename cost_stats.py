...
def analyze_jsonl_files():
    ...
    for filepath in sorted(files):
                ...
                try:
                    ...
                    if data.get("type") == "message" and "message" in data:
                        ...
                        if "usage" in msg:
                            ...
                            total_stats["cache_read_tokens"] += usage.get(
                                "cacheRead", 0
                            )
                            total_stats["cache_write_tokens"] += usage.get(
                                "cacheWrite", 0
                            )
                            ...
                            if ts:
                                ...
                                daily_stats[day_key]["tokens"] += usage.get(
                                    "totalTokens", 0
                                )
                                ...
                                hourly_stats[hour_key]["tokens"] += usage.get(
                                    "totalTokens", 0
                                )
        ...
        total_stats["files"] += 1
        duration = (
            (file_end - file_start).total_seconds() if file_start and file_end else 0
        )
        file_stats.append(
            {
    ...
    if model_stats:
        ...
        for model, stats in sorted(model_stats.items(), key=lambda x: -x[1]["cost"]):
            print(f"   {model}:")
            print(
                f"      Messages: {stats['messages']:,}  |  Tokens: {stats['tokens']:,}  |  Cost: ${stats['cost']:.4f}"
            )
    ...
    for fs in sorted(file_stats, key=lambda x: -x["cost"]):
        ...
        print(
            f"   {start_str}  {duration_str:>6}  ${fs['cost']:>8.4f}  ({fs['messages']:>3} msgs)"
        )
